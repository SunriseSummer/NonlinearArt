# 神经网络在临界态时有最佳学习能力：基于 MLP + Fashion-MNIST 的实证研究
...
**关键词：** 临界态；混沌边缘；深层前馈网络；ReLU；Fashion-MNIST

---

## 1. 引言

本文将通过实验验证猜想“神经网络处于临界态/混沌边缘时学习效率最高”。不同架构对临界态指标的响应存在差异：在残差结构中，经典阈值可能偏移；在非残差深层网络中，经典理论预期更容易直接验证。
...
使用 Fashion-MNIST（训练 60 000 / 测试 10 000），输入灰度图大小为 $28 \times 28$，展平为 784 维向量，类别数为 10。像素按 $x \mapsto x/255$ 从 $[0,255]$ 线性归一化到 $[0,1]$。
...
- $\sigma_b$：相邻层激活范数的平均放大倍数，用于刻画信号是衰减、守恒还是放大。
- $\lambda$：微小扰动随深度传播的指数增长率，$\lambda < 0$ 表示收敛，$\lambda > 0$ 表示发散，$\lambda \approx 0$ 对应边缘混沌。
- $\tau$ 与 $R^2$：用于描述雪崩规模分布的幂律拟合形状与拟合质量。
- PR：隐藏表示的有效维数，反映表示空间是否退化或坍缩。

### 2.5 相区定义

为避免术语歧义，本文采用基于动力学稳定性的相区划分：

- **有序相（ordered）**：$\lambda < 0$，扰动随层数衰减，信号趋于收敛；
- **临界相（critical）**：$\lambda \approx 0$，扰动既不明显衰减也不明显发散；
- **混沌相（chaotic）**：$\lambda > 0$，扰动随层数指数放大。

文中示例点 $\sigma=0.30$、$1.15$、$2.50$ 分别作为三类相区的代表配置。
